...
[Api(3)]
public sealed class SMN_LeliaDefaultPvE1 : SummonerRotation
{
    ...
    protected override bool AttackAbility(IAction nextGCD, out IAction? act)
    {
        ...
        {
            if (PainflarePvE.CanUse(out act)) return true;
            if (FesterPvE.CanUse(out act)) return true;
        }

        if (Player.HasStatus(true, StatusID.SearingLight))
        {
            if (PainflarePvE.CanUse(out act)) return true;
            if (FesterPvE.CanUse(out act)) return true;
        }
    ...
    protected override bool EmergencyAbility(IAction nextGCD, out IAction? act)
    {
        ...
        if ((RadiantOnCooldown && RadiantAegisPvE.Cooldown.CurrentCharges == 2 || RadiantAegisPvE.Cooldown.CurrentCharges == 1 && RadiantAegisPvE.Cooldown.WillHaveOneCharge(5)) && 
            (anyBigInvocationIsCoolingDown && Player.Level <= 100) && Player.GetHealthRatio() <0.8f && RadiantAegisPvE.CanUse(out act)) return true;
        if (RadiantOnCooldown && Player.Level < 88 && Player.GetHealthRatio() < 0.8f && anyBigInvocationIsCoolingDown && RadiantAegisPvE.CanUse(out act)) return true;

...
[Rotation("NIN_Low(1-44)", CombatType.PvE, GameVersion = "7.05")]
[SourceCode(Path = "main/DefaultRotations/Melee/NIN_Default.cs")]
[Api(4)]

public sealed class NIN_DefaultLow : NinjaRotation
{
    ...
    private bool ChoiceNinjutsu(out IAction? act)
    {
        ...
        if (!ChiPvE.CanUse(out _) || !TenPvE.CanUse(out _)) return false;
        ...
        if (InCombat && Player.HasStatus(true, StatusID.Kassatsu))
        {
    ...
    private bool DoNinjutsu(out IAction? act)
    {
        ...
        //TenChiJin
        if (InCombat && Player.HasStatus(true, StatusID.TenChiJin))
        {
        ...
        if (InCombat && !Player.WillStatusEnd(3, false, StatusID.Kassatsu)
        ...
        //First
        else if (InCombat && id == ActionID.NinjutsuPvE)
        {
            //Can't use.
        ...
        //Second
        else if (InCombat && (uint)id == _ninActionAim.ID)
        {
        ...
        //Third
        else if (InCombat && (uint)id == FumaShurikenPvE.ID)
        {
        ...
        //Finished
        else if (InCombat && (uint)id == KatonPvE.ID || (uint)id == RaitonPvE.ID || (uint)id == HyotonPvE.ID)
        {

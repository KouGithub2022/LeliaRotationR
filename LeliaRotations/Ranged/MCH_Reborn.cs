...
public sealed class MCH_LeliaRotation : MachinistRotation
{
    ...
    protected override bool GeneralGCD(out IAction? act)
    {
        ...
        if (CurrentTarget is not null && InCombat && UseLBMaxPvE && MyLimitBreakLevel >= LimitBreakMax && !IsDead && Target.DistanceToPlayer() < 20)
        {
            Chat.ExecuteCommand("/action リミットブレイク <t>");
        }

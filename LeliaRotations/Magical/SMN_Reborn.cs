...
public sealed class SMN_LeliaRotation : SummonerRotation
{
    ...
    protected override bool AttackAbility(IAction nextGCD, out IAction? act)
    {
        ...
        if ((RadiantOnCooldown && RadiantAegisPvE.Cooldown.CurrentCharges == 2 || RadiantAegisPvE.Cooldown.CurrentCharges == 1) && 
            RadiantAegisPvE.Cooldown.WillHaveOneCharge(5) && Player.GetHealthRatio() <= 0.8f && Player.Level <= 100 && RadiantAegisPvE.CanUse(out act))
        {
            return true;
        }

        if (RadiantOnCooldown && Player.Level < 88 && RadiantAegisPvE.Cooldown.CurrentCharges == 1 && Player.GetHealthRatio() <= 0.8f && RadiantAegisPvE.CanUse(out act))
        {
            return true;
        }

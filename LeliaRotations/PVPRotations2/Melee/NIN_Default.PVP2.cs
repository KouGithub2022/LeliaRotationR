...
public sealed class NIN_DefaultPvP2 : NinjaRotation
{
    ...
    protected override bool GeneralGCD(out IAction? action)
    {
        ...
        //if (CurrentTarget is not null && UseLB && MyLimitBreakLevel == 1 &&  Target.GetHealthRatio() <= LBValue && SeitonTenchuPvP.CanUse(out action, skipAoeCheck: true, usedUp: true)) return true;

        if (UseShukuchi && Target.CurrentHp <= ShukuchiValue && !ShukuchiPvP.Cooldown.IsCoolingDown)
        {

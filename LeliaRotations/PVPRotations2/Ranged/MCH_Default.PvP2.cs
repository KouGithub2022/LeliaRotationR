...
//[Api(6)]
public sealed class MCH_DefaultPvP2 : MachinistRotation
{
    ...
    public int LBValue1 { get; set; } = 35000;
    ...
    protected override bool AttackAbility(IAction nextGCD, out IAction? action)
    {
        ...
        if (InCombat && BraveryPvP.CanUse(out action) && !BraveryPvP.Cooldown.IsCoolingDown && 
            nextGCD.IsTheSameTo(true, (ActionID)MarksmansSpitePvP.ID,(ActionID)FullMetalFieldPvP.ID,(ActionID)WildfirePvP.ID,(ActionID)AnalysisPvP.ID))
        {
            return true;
        }
    ...
    protected override bool GeneralGCD(out IAction? action)
    {
        ...
        if (CurrentTarget is not null && UseLB && MyLimitBreakLevel == 1 && Target.CurrentHp <= LBValue1 && MarksmansSpitePvP.CanUse(out action)) return true;

        if (FullMetalFieldPvP.CanUse(out action,skipAoeCheck: true) && !FullMetalFieldPvP.Cooldown.IsCoolingDown)
        {
            return true;
        }

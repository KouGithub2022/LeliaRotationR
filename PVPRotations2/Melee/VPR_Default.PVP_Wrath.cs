...
//[Api(6)]
public sealed class VPR_DefaultPvPwrath : ViperRotation
{
    public static IBaseAction WorldswallowerPvP = new BaseAction((ActionID)39190);
    public static IBaseAction SlitherPvP = new BaseAction((ActionID)39184);
    ...
    [RotationConfig(CombatType.PvP, Name = "蛇行を使用します")]
    public bool UseSlither { get; set; } = false;
    ...
    protected override bool AttackAbility(IAction nextGCD, out IAction? action)
    {
        ...
        if (UseSlither && Target.DistanceToPlayer() > 5 && !SlitherPvP.Cooldown.IsCoolingDown && SlitherPvP.CanUse(out action, usedUp: true)) return true;
    ...
    protected override bool GeneralGCD(out IAction? action)
    {
        ...
        if (!Player.HasStatus(true,StatusID.SnakesBane) && UseLB && MyLimitBreakLevel == 1 && Target.CurrentHp <= LBValue && WorldswallowerPvP.CanUse(out action, skipAoeCheck: true, usedUp: true)) return true;
